...
window.onload = function () {
    ...
    function rightBottomData() {
        ...
        for (let i = 0; i < arr.length; i++) {
            ...
            dl.appendChild(dt);
            dt.innerHTML=arr[i].title
            for(let j=0;j<arr[i].data.length;j++){
                var dd =document.createElement('dd');
                dd.innerHTML=arr[i].data[j].type
                dd.setAttribute('price',arr[i].data[j].changePrice)
                dl.appendChild(dd)
            }
    ...
    function clickddBind(){
        ...
        let arr = document.querySelectorAll('.chooseWrap dl');
        for(let a=0;a<arr.length;a++){
            let arr1 = arr[a].querySelectorAll('dd');
    
            arr1.forEach(dd => {
                dd.onclick = function() {
                    
                    // 点击时重置所有 dd 元素的颜色为基础颜色
                    arr1.forEach(dd => {
                        dd.style.color = '#666';
                    });
                    // 将当前点击的 dd 元素颜色设置为红色
                    ...
                    arrays[a]=this;
                    changePriceBind(arrays);
                    var choose=document.querySelector("#choose");
                    choose.innerHTML='';
                    arrays.forEach((t,index)=>{
                        if(t!=0){
                            const div = document.createElement('div');
                        div.className = "mark"; // 使用 className 直接设置类名，注意不要用 add
                        // div.innerHTML = t + `<a class="remove">X</a>`; 
                        ...
                        aNode.onclick=function(){
                            ...
                            choose.removeChild(div)
                            let arr2=arr[number].querySelectorAll('dd');
                            console.log(arr2)
                            arr2.forEach(t=>{
                                t.style.color='#666'
                            })
                            arr2[0].style.color='red'
                            arrays[number]=0;
                            changePriceBind(arrays)
                        }
     ...
     */
    function changePriceBind(arrays){
        /**
         * 思路：
         * 1、获取价格的标签元素
        ...
        arrays.forEach(t=>{
            if(t!=0){
                newprice+=Number(t.getAttribute('price'))
            }
        })
        oldPrice.innerHTML=Number(price)+Number(newprice)

        //3、将变化后的价格写入到左侧标签中
        var leftprice = document.querySelector('#wrapper #content .contentMain .goodsDetailWrap .rightDetail .chooseBox .listWrap .left p');


        leftprice.innerText = '¥' + oldPrice.innerHTML;
        let price2=oldPrice.innerHTML;
        let price3=0;
        ...
        var newprice2 = document.querySelector('#wrapper #content .contentMain .goodsDetailWrap .rightDetail .chooseBox .listWrap .right i');
        
        for(let j = 0;j < ipts.length;j++){
            if(ipts[j].checked){
                price3=Number(ipts[j].value);
            }
        }

        // console.log(newprice2.innerText)
        // console.log(price2)
        // // //5、右侧的套餐价价格重新渲染
        newprice2.innerText = '¥' + (Number(price2)+Number(price3));
    }
                ...
                for(let j = 0 ; j < ipts.length;j++){
                    if(ipts[j].checked){
                          ...
                          oldprice = oldprice + Number(ipts[j].value);
                            ...
                            console.log(Number(ipts[j].value))
                    }
                }
